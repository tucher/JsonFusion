...
struct FindAndEval : public RecursiveASTVisitor<FindAndEval> {
  ...
  bool VisitVarDecl(VarDecl *VD) {
    ...
    if (!VD || !VD->isConstexpr()) return true;
    if (VD->getName() != Name) return true;
    ...
    if (!Init->EvaluateAsRValue(ER, Ctx)) {
      Error = "failed to evaluate constexpr initializer";
      Found = true;
      return true;
    }
  ...
  bool VisitFunctionDecl(FunctionDecl *FD) {
    if (Found) return true;
    if (!FD || FD->getName() != Name) return true;
    if (!(FD->isConstexpr() || FD->isConsteval())) return true;
...
int main(int argc, const char **argv) {
  ...
  Args.push_back("-isystem"); Args.push_back("/sysroot/include/c++/v1");
  Args.push_back("-isystem"); Args.push_back("/sysroot/lib/clang/21/include");
  Args.push_back("-isystem"); Args.push_back("/sysroot/include");
#else
